...
const AccommodationsTab = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (accommodation: any) => {
      const { contact_person, contact_email, contact_phone, ...rest } = accommodation;
      const { error } = await supabase
        .from("accommodations")
        .update(rest)
      ...
      // Upsert contact details to auth-gated side table
  ...
  const loadContacts = async (acc: any) => {
  ...
  const handleEdit = async (acc: any) => {
    setSelectedAccommodation(await loadContacts(acc));
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <form onSubmit={handleUpdate} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="distance_from_university_km">Distance to Uni (km)</Label>
                <Input
                  id="distance_from_university_km"
                  type="number"
                  step="0.1"
                  value={selectedAccommodation?.distance_from_university_km || ""}
                  onChange={(e) => setSelectedAccommodation({ ...selectedAccommodation, distance_from_university_km: parseFloat(e.target.value) })}
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={selectedAccommodation?.accreditation_number || ""}
                  onChange={(e) => setSelectedAccommodation({ ...selectedAccommodation, accreditation_number: e.target.value })}
                />
              </div>
              <div className="space-y-2 col-span-2">
                <Label htmlFor="nearby_shops">Nearby Shops</Label>
                <Textarea
                  id="nearby_shops"
                  value={selectedAccommodation?.nearby_shops || ""}
                  onChange={(e) => setSelectedAccommodation({ ...selectedAccommodation, nearby_shops: e.target.value })}
                  rows={2}
                />
              </div>
              <div className="space-y-2 col-span-2">
                <Label htmlFor="certified_universities">Certified Universities (comma-separated)</Label>
                <Input
                  id="certified_universities"
                  value={selectedAccommodation?.certified_universities?.join(", ") || ""}
                  onChange={(e) => setSelectedAccommodation({ ...selectedAccommodation, certified_universities: e.target.value.split(",").map(v => v.trim()).filter(Boolean) })}
                />
              </div>
              <div className="space-y-2 col-span-2">
        ...
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-muted-foreground">Distance to Uni</p>
                <p>{selectedAccommodation?.distance_from_university_km ? `${selectedAccommodation.distance_from_university_km}km` : "N/A"}</p>
              </div>
              <div>
                <p className="text-sm font-medium text-muted-foreground">Accreditation Number</p>
                <p>{selectedAccommodation?.accreditation_number || "N/A"}</p>
              </div>
            </div>
            <div>
              <p className="text-sm font-medium text-muted-foreground">Nearby Shops</p>
              <p className="text-sm">{selectedAccommodation?.nearby_shops || "N/A"}</p>
            </div>
            <div>
              <p className="text-sm font-medium text-muted-foreground">Certified For</p>
              <div className="flex flex-wrap gap-1 mt-1">
                {selectedAccommodation?.certified_universities?.length > 0 ? (
                  selectedAccommodation.certified_universities.map((uni: string) => (
                    <Badge key={uni} variant="outline" className="text-[10px]">{uni}</Badge>
                  ))
                ) : (
